...
public:
    ...
    //      -Move towards our home
        ...
        if (closest != NULL) {
          ...
          if (ratio > 1.0) {
            ...
            modx = ratio;
					...
					if(closestRobot == NULL ||(closestRobot != NULL && (relDistance(closestRobot->relx, closestRobot->rely) > 1.5*ROBOTDIAMETER))){
		        ...
		        if (closest->rely <= 0.0) {
		          ...
		          command.setVy(velocity * mody);
						...
						SeenPuck* secondClosest = findSecondClosestPuck(ownRobot);
						if (secondClosest != NULL) {
				      // We can see at least one puck!
				      if(secondClosest->rely == 0)  //prevent NaN
				      secondClosest->rely = 0.000001;
				      double ratio = abs(secondClosest->relx / secondClosest->rely);
				      double modx = 1.0;
				      double mody = 1.0;
				      if (ratio > 1.0) {
				        mody = 1.0 / ratio;
      ...
      //do /3.5 to ensure we're really inside the home before dropping. None of this border crap.
			if (relDistance(ownRobot->homeRelX, ownRobot->homeRelY) < HOMEDIAMETER/2.2) {
        ...
        if (ratio > 1.0) {
          ...
          modx = ratio;
		      ...
		      if (ownRobot->homeRelY <= 0.0) {
				...
				else if((closestRobot != NULL && (relDistance(closestRobot->relx, closestRobot->rely) < 1.5*ROBOTDIAMETER))){
