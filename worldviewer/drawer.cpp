...
void UnpackImage(cairo_t *cr, RegionRender* render, int robotSize, double robotAlpha, WorldInfo *worldinfo, unsigned int regionId) {
	...
	for (int i = 0; i < worldinfo->home_size(); i++) {
		...
		if (homeInfo->region_id() == regionId) {
			...
			cairo_arc(cr, IMAGEWIDTH*((double)homeInfo->home_x()/(double)REGIONSIDELEN), IMAGEHEIGHT*((double)homeInfo->home_y()/(double)REGIONSIDELEN), HOMEDIAMETER/ 2, 0, 2 * M_PI);
			cairo_stroke(cr);
		}
	}
	...
	for (int i = 0; i < render->image_size(); i++) {
		...
		if (robotSize == 1) {
			...
			cairo_pattern_add_color_stop_rgba(radpat, (double) robotSize / 2.0, color.r, color.g,
					color.b, 0.0);
